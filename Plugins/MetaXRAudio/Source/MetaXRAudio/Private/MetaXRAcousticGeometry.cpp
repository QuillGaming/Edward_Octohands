...
void UMetaXRAcousticGeometry::CheckGeoTransformValid() {
  ...
  if (!Owner->GetTransform().Equals(GetComponentTransform())) {
    ...
    METAXR_AUDIO_LOG_WARNING(
        "You cannot change the transform of the UGeometry on %s, only the transform of your static mesh or landscape. This ensures the audio geometry stays in sync with the physical geometry.",
...
void UMetaXRAcousticGeometry::BeginPlay() {
  ...
  if (!StartInternal()) {
    const FString ComponentPath = MetaXRAudioUtilities::GetComponentPathInScene(*this);
    METAXR_AUDIO_LOG_ERROR("Failed to initialize Acoustic Geometry. Component will not function. Component Path: %s", *ComponentPath);
...
bool UMetaXRAcousticGeometry::StartInternal() {
  if (!CreatePropagationGeometry()) {
    // ovrAudio_CreateAudioGeometry could be called but we failed to upload geometry.
    // We should not keep the OvrGeo around as it has not use here.
    DestroyPropagationGeometry();
    const FString ComponentPathInfo = MetaXRAudioUtilities::GetComponentPathInScene(*this);
    METAXR_AUDIO_LOG_ERROR(
        "Failed to create propagation geometry. This component will not function. Path to Component:\n%s", *ComponentPathInfo);
    return false;
  }
  ApplyTransform();
  return true;
...
bool UMetaXRAcousticGeometry::ReadFile() {
  if (FilePath.IsEmpty()) {
    const AActor* Owner = GetOwner();
    const FString NoPathOwner = Owner ? *Owner->GetActorNameOrLabel() : FString("Invalid Owner");
    METAXR_AUDIO_LOG_WARNING("No geometry file path provided for %s!", *NoPathOwner);
    return false;
  }

  if (IsPlaymodeActive()) {
    if (!LoadGeometryFromFile())
      return false;
  } else {
...
void DebugFileInfo(const FString& RelativePath) {
  const FString FullFilePath = FPaths::ProjectContentDir() / RelativePath;
  if (!FPaths::FileExists(FullFilePath)) {
    METAXR_AUDIO_LOG_WARNING("Audio geometry file not found: %s", *FullFilePath);
    return;
  }

  FFileStatData FileInfo;
  MetaXRAudioUtilities::GetFileStatData(RelativePath, FileInfo);
  const FString DebugInfo = MetaXRAudioUtilities::GetFileDebugInfo(RelativePath, FileInfo);
  METAXR_AUDIO_LOG_ERROR("%s", *DebugInfo);
}

bool UMetaXRAcousticGeometry::LoadGeometryFromFile() {
  const FString FullFilePath = FPaths::ProjectContentDir() / FilePath;
...
void FAcousticGeoGizmoData::UpdateGizmoMeshData(ovrAudioGeometry GeometryHandle) {
  ...
  uint32_t NumVertices = 0, NumTriangles = 0;
  ...
  if (Result != ovrSuccess || NumVertices <= 0 || NumTriangles <= 0) {
    METAXR_AUDIO_LOG_WARNING("(0) Failed getting simplified mesh from the audio propagation sub-system!");
    return;
  }
  ...
  if (Result != ovrSuccess) {
    METAXR_AUDIO_LOG_WARNING("(1) Failed getting simplified mesh from the audio propagation sub-system!");
    return;
  }
  ...
  for (int i = 0; i < IndicesArray.Num(); i += 3) {
    ...
    UMetaXRAcousticMaterialProperties* AcousticMaterial =
        GizmoMaterialMapping.IsValidIndex(MaterialIndex) ? GizmoMaterialMapping[MaterialIndex] : nullptr;

...
class MetaXRAudioUtilities {
 public:
  ...
  static FString GetFileDebugInfo(const FString& RelativeFilePath, const FFileStatData& FileStatData) {
    const FString FullFilePath = FPaths::ProjectContentDir() / RelativeFilePath;
    const FString DebugInfoStr = FString::Printf(
        TEXT(
            "{ \"FilePath\": \"%s\", \"FileSize\": %lld, \"IsDirectory\": %s, \"IsReadOnly\": %s, \"IsValid\": %s, \"CreationTime\": \"%s\", \"AccessTime\": \"%s\", \"ModificationTime\": \"%s\" }"),
        *FullFilePath,
        FileStatData.FileSize,
        FileStatData.bIsDirectory ? TEXT("true") : TEXT("false"),
        FileStatData.bIsReadOnly ? TEXT("true") : TEXT("false"),
        FileStatData.bIsValid ? TEXT("true") : TEXT("false"),
        *FileStatData.CreationTime.ToString(),
        *FileStatData.AccessTime.ToString(),
        *FileStatData.ModificationTime.ToString());
    return DebugInfoStr;
  }

  static FString GetComponentPathInScene(const UActorComponent& ActorComponent) {
    TArray<FString> PathElements;

    PathElements.Add(ActorComponent.GetName());

    const USceneComponent* SceneComp = Cast<USceneComponent>(&ActorComponent);
    if (SceneComp) {
      const USceneComponent* ParentComp = SceneComp->GetAttachParent();
      while (ParentComp) {
        PathElements.Add(ParentComp->GetName());
        ParentComp = ParentComp->GetAttachParent();
      }
    }

    AActor* CurrentActor = ActorComponent.GetOwner();
    while (CurrentActor) {
      PathElements.Add(CurrentActor->GetActorNameOrLabel());
      CurrentActor = CurrentActor->GetAttachParentActor();
    }

    return FString::Join(PathElements, TEXT("/"));
  }
